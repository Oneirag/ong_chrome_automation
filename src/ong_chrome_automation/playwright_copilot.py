...
class CopilotChatAutomation:
    ...
    CODE_LOCATOR = ".scriptor-paragraph"                # For finding code blocks
    ...
    def __init__(self, browser, url: str = None, use_work: bool=True):
        url = url or self.URL
    ...
    def new_chat(self):
        ...
        if self.use_work:
            try:
                ...
                print("No work button found, using web/chat")
        else:
            try:
    ...
    def get_response_code_blocks(self) -> List[str]:
        ...
        for iframe in iframes:
            all_code_lines = code.locator(self.CODE_LOCATOR)
            code = "\n".join([
...
class M365CopilotAutomation(CopilotChatAutomation):
    ...
    def get_response_code_blocks(self) -> List[str]:
        ...
        for n_code in range(response_codes.count()):
            all_code_lines = response_codes.nth(n_code).locator(self.CODE_LOCATOR)
            code = "\n".join([
                all_code_lines.nth(i).inner_text() for i in range(1, all_code_lines.count())
            ])
            all_codes.append(code)
